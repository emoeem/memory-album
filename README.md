...
一份写给朋友的回忆录：打开是她的名字，点开之后音乐进来，往下滚，时间线带你走过你们经历过的事，最后是祝福和鼓励。

**一个链接只能看到一个人的内容。** 目前有 `yales`（测试数据）。

视觉参考了《天气之子》：雨夜的深蓝打底，一路下雨，走到结尾那一段雨会停下来、天色转暖——放晴。对应的是 RADWIMPS 的『グランドエスケープ』。

## 叙事过程

整篇是一格一格推下去的，参考了那类分镜式祝福页的做法：

1. **遮罩开场** —— 她的名字 + 「温馨提醒：戴上耳机听会更好」+「开始」
2. **章节** —— 时间线在侧边（手机在顶部）跟着亮起来，图片按你排的顺序出现
3. **停顿页** —— 一整屏只放一句话，**一个字一个字浮出来**，给整篇留呼吸
4. **聊天分镜** —— 一个聊天气泡，滚到时话一个字一个字打出来，最后冒出「发送」
5. **歌单** —— 一起听过的歌
6. **放晴** —— 越接近结尾雨越小、暖光越亮
7. **再看一遍** —— 结尾按钮会把动画和音乐全部倒回开头

每一幕的进出动作是**跟着滚动位置算的，不是定时器**：斜着落下来 → 停住 → 斜着滑走、上飞、轻微缩小。所以她停在哪一幕，哪一幕就停在那儿，不用赶时间。

同一个节点里，**图片和文字分成两拍**：图片先落下来，文字晚半拍跟上（`data-lag`）。比一屏还高的元素不做斜切、也不做退场——长截图一斜，上边缘会横着偏出屏幕一百多像素。
